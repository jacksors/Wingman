...
import { Review, User, TailNumber } from '@prisma/client';
import { ModeToggle } from '@/components/mode-toggle';
import { Card } from '@/components/ui/card';
const inter = Inter({ subsets: ['latin'] });

export default function PlaneReviews() {
	const { id } = useRouter().query;
	const [reviews, setReviews] = useState<Review[]>([]);
  const [averageRating, setAverageRating] = useState<number>(0);
	const [tailNumber, setTailNumber] = useState<TailNumber>({ id: -1, number: '', model: '', airlineId: -1 });
        ...
        fetch(`/api/planes/${id}/reviews`, {
            ...
            if (res.status === 200) {
                res.json().then(async (data: Review[]) => {
                    ...
                    if (data.length !== 0) setAverageRating(Math.round(sum / data.length))
	...
	return (
			...
			<h1 className='text-5xl text-center pt-4'>Reviews for {tailNumber.model} ({tailNumber.number})</h1>
      {
        ...
        <ul className='grid grid-cols-1 md:grid-cols-2 gap-7 py-3 w-3/4 '>
          {reviews.map((review) => (
            <Card key={review.id} className='flex flex-col items-center gap-2 p-4 bg-secondary hover:scale-105 transition-transform'>
              {/* Title */}
              <div className='flex flex-col w-full items-center justify-center'>
                <h2 className='text-2xl text-center'>{review.title}</h2>
                <p>{review.content}</p>
              </div>
              {/* Star rating */}
              <div className='flex flex-row gap-2'>
